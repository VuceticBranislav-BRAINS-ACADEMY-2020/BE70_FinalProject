...
public interface JoinTableSubjectTeacherRepository extends CrudRepository<JoinTableSubjectTeacher, Integer> {
	...
	JoinTableSubjectTeacher findByTeachersAndSubclsAndGroup(@Param("teacher") TeacherEntity teacher,
			@Param("sub_cls") JoinTableSubjectClass sub_cls, @Param("group") GroupEntity group);

	@Override
	List<JoinTableSubjectTeacher> findAll();

	@Override
	Optional<JoinTableSubjectTeacher> findById(Integer id);
}
