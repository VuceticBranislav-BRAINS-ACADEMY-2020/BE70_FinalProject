...
public interface AddressRepository extends CrudRepository<AddressEntity, Integer> {
	...
	@Override
	@Query(value = "FROM address AS a WHERE a.id=:id AND a.status<>'DELETED'")
	Optional<AddressEntity> findById(@Param("id") Integer id);
	 
	@Override
	Set<AddressEntity> findAll();
	
	@Query(value = "SELECT COUNT(*)>0 FROM address AS a WHERE a.city = :city AND a.street = :street AND a.number = :number AND a.apartment = :apartment")
	...
	@Query(value = "SELECT COUNT(*)>0 FROM address AS a WHERE a.city = :city AND a.street = :street AND a.number = :number")
	Boolean isAddressExists(@Param("city") String city, @Param("street") String street, @Param("number") String number);
	
	@Query(value = "FROM address AS a WHERE a.status<>'DELETED'")
